...
{% endhighlight %}
    ...
    <tr>
      ...
      <td>
        <p>
          以 round-robin 的形式将元素分区到下游操作的子集中。如果你想要将数据从一个源的每个并行实例中散发到一些 mappers 的子集中，用来分散负载，但是又不想要完全的 rebalance 介入（引入 <code>rebalance()</code>），这会非常有用。根据一些如TaskManager 的 slots 个数的配置，这将会只需要本地数据传输，而不是通过网络。
        </p>
        <p>
